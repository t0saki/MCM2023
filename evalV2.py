...
with open('means_stds.txt', 'r') as f:
    means_stds = f.read()
...
model.load_state_dict(torch.load("/mnt/d/checkpoints/wordleV2.1_50epochs 1.5653e-02"))
...
outputs = pd.DataFrame(outputs, columns=target_columns)

# Write raw outputs to csv
outputs.to_csv('raw_outputs.csv', index=False)
